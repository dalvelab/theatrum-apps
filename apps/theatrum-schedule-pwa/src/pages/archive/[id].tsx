...
import { CardSchedule, getArchivedScheduleByMonth, ModalSchedule, getScheduleByDays } from '@/entities';
import type { ScheduleEvent } from '@/entities';
import { Loader } from '@/components';

export default function ArchivedEventsForSingleMonth({ events }: InferGetServerSidePropsType<typeof getServerSideProps>) {
...
export const getServerSideProps: GetServerSideProps<ArchiveProps> = async ({ params }) => {
  ...
  const events = await getArchivedScheduleByMonth({ id, limit: 100 });
